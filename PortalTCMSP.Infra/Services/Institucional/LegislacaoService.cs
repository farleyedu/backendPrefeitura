...
{
    public class LegislacaoService : ILegislacaoService
    {
        private readonly IInstitucionalRepository _repo;
        private readonly PortalTCMSPContext _ctx;

        public LegislacaoService(IInstitucionalRepository repo, PortalTCMSPContext ctx)
        {
            _repo = repo;
            _ctx = ctx;
        }

        public async Task<LegislacaoResponse?> GetAsync(long id, CancellationToken ct = default)
        {
            var e = await _repo.GetByIdWithTreeAsync(id, ct);
            return e is null ? null : e.ToResponse().ToLegislacaoResponse();
        }

        public async Task<LegislacaoResponse?> GetBySlugAsync(string slug, CancellationToken ct = default)
        {
            var e = await _repo.GetBySlugWithTreeAsync(slug, ct);
            return e is null ? null : e.ToResponse().ToLegislacaoResponse();
        }

        public async Task<List<LegislacaoResponse>> SearchAsync(string? ativo, DateTime? publicadoAte, CancellationToken ct = default)
        {
            var list = await _repo.SearchAsync(ativo, publicadoAte, ct);
            return list.Select(x => x.ToResponse()).ToLegislacaoResponse();
        }

        public async Task<long> CreateAsync(CreateLegislacaoRequest req, CancellationToken ct = default)
        {
            var instCreate = req.ToInstitucionalCreate();

            // slug único (usa título se vier vazio)
        ...
        public async Task UpdateAsync(long id, UpdateLegislacaoRequest req, CancellationToken ct = default)
        {
