...
{
    public class PlanejamentoEstrategicoService : IPlanejamentoEstrategicoService
    {
        private readonly IInstitucionalRepository _repo;
        private readonly PortalTCMSPContext _ctx;

        public PlanejamentoEstrategicoService(IInstitucionalRepository repo, PortalTCMSPContext ctx)
        {
            _repo = repo;
            _ctx = ctx;
        }

        public async Task<PlanejamentoEstrategicoResponse?> GetAsync(long id, CancellationToken ct = default)
        {
            var e = await _repo.GetByIdWithTreeAsync(id, ct);
            return e is null ? null : e.ToResponse().ToPlanejamentoEstrategicoResponse();
        }

        public async Task<PlanejamentoEstrategicoResponse?> GetBySlugAsync(string slug, CancellationToken ct = default)
        {
            var e = await _repo.GetBySlugWithTreeAsync(slug, ct);
            return e is null ? null : e.ToResponse().ToPlanejamentoEstrategicoResponse();
        }

        public async Task<List<PlanejamentoEstrategicoResponse>> SearchAsync(string? ativo, DateTime? publicadoAte, CancellationToken ct = default)
        {
            var list = await _repo.SearchAsync(ativo, publicadoAte, ct);
            return list.Select(x => x.ToResponse()).ToPlanejamentoEstrategicoResponse();
        }

        public async Task<long> CreateAsync(CreatePlanejamentoEstrategicoRequest req, CancellationToken ct = default)
        {
        ...
        public async Task UpdateAsync(long id, UpdatePlanejamentoEstrategicoRequest req, CancellationToken ct = default)
        {
