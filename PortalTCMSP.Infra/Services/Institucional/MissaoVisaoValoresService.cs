...
{
    public class MissaoVisaoValoresService : IMissaoVisaoValoresService
    {
        private readonly IInstitucionalRepository _repo;
        private readonly PortalTCMSPContext _ctx;

        public MissaoVisaoValoresService(IInstitucionalRepository repo, PortalTCMSPContext ctx)
        {
            _repo = repo;
            _ctx = ctx;
        }

        public async Task<MissaoVisaoValoresResponse?> GetAsync(long id, CancellationToken ct = default)
        {
            var e = await _repo.GetByIdWithTreeAsync(id, ct);
            return e is null ? null : e.ToResponse().ToMissaoVisaoValoresResponse();
        }

        public async Task<MissaoVisaoValoresResponse?> GetBySlugAsync(string slug, CancellationToken ct = default)
        {
            var e = await _repo.GetBySlugWithTreeAsync(slug, ct);
            return e is null ? null : e.ToResponse().ToMissaoVisaoValoresResponse();
        }

        public async Task<List<MissaoVisaoValoresResponse>> SearchAsync(string? Ativo, DateTime? publicadoAte, CancellationToken ct = default)
        {
            var list = await _repo.SearchAsync(Ativo, publicadoAte, ct);
            return list.Select(x => x.ToResponse()).ToMissaoVisaoValoresResponse();
        }

        public async Task<long> CreateAsync(CreateMissaoVisaoValoresRequest req, CancellationToken ct = default)
        {
        ...
        public async Task UpdateAsync(long id, UpdateMissaoVisaoValoresRequest req, CancellationToken ct = default)
        {
