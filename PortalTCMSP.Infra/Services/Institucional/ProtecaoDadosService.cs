...
{
    public class ProtecaoDadosService : IProtecaoDadosService
    {
        private readonly IInstitucionalRepository _repo;
        private readonly PortalTCMSPContext _ctx;

        public ProtecaoDadosService(IInstitucionalRepository repo, PortalTCMSPContext ctx)
        {
            _repo = repo;
            _ctx = ctx;
        }

        public async Task<ProtecaoDadosResponse?> GetAsync(long id, CancellationToken ct = default)
        {
            var e = await _repo.GetByIdWithTreeAsync(id, ct);
            return e is null ? null : e.ToResponse().ToProtecaoDadosResponse();
        }

        public async Task<ProtecaoDadosResponse?> GetBySlugAsync(string slug, CancellationToken ct = default)
        {
            var e = await _repo.GetBySlugWithTreeAsync(slug, ct);
            return e is null ? null : e.ToResponse().ToProtecaoDadosResponse();
        }

        public async Task<List<ProtecaoDadosResponse>> SearchAsync(string? ativo, DateTime? publicadoAte, CancellationToken ct = default)
        {
            var list = await _repo.SearchAsync(ativo, publicadoAte, ct);
            return list.Select(x => x.ToResponse()).ToProtecaoDadosResponse();
        }

        public async Task<long> CreateAsync(CreateProtecaoDadosRequest req, CancellationToken ct = default)
        {
        ...
        public async Task UpdateAsync(long id, UpdateProtecaoDadosRequest req, CancellationToken ct = default)
        {
