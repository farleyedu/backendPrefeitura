...
namespace PortalTCMSP.Infra.Services.Institucional
{
    public class HistoriaService : IHistoriaService
    {
        ...
        public HistoriaService(IInstitucionalRepository repo, PortalTCMSPContext ctx)
        {
            _repo = repo;
            _ctx = ctx;
        }

        public async Task<HistoriaResponse?> GetAsync(long id, CancellationToken ct = default)
        {
            var e = await _repo.GetByIdWithTreeAsync(id, ct);
            return e is null ? null : e.ToResponse().ToHistoriaResponse();
        }

        public async Task<HistoriaResponse?> GetBySlugAsync(string slug, CancellationToken ct = default)
        {
            var e = await _repo.GetBySlugWithTreeAsync(slug, ct);
            return e is null ? null : e.ToResponse().ToHistoriaResponse();
        }

        public async Task<List<HistoriaResponse>> SearchAsync(string? ativo, DateTime? publicadoAte, CancellationToken ct = default)
        {
            var list = await _repo.SearchAsync(ativo, publicadoAte, ct);
            return list.Select(x => x.ToResponse()).ToHistoriaResponse();
        }

        public async Task<long> CreateAsync(CreateHistoriaRequest req, CancellationToken ct = default)
        {
        ...
        public async Task UpdateAsync(long id, UpdateHistoriaRequest req, CancellationToken ct = default)
        {
