...
namespace PortalTCMSP.Infra.Services.Home
{
    public class OrganogramaService : IOrganogramaService
    {
        ...
        public OrganogramaService(IInstitucionalRepository repo, PortalTCMSPContext ctx)
        {
            _repo = repo;
            _ctx = ctx;
        }

        public async Task<OrganogramaResponse?> GetAsync(long id, CancellationToken ct = default)
        {
            var e = await _repo.GetByIdWithTreeAsync(id, ct);
            return e is null ? null : e.ToResponse().ToOrganogramaResponse();
        }

        public async Task<OrganogramaResponse?> GetBySlugAsync(string slug, CancellationToken ct = default)
        {
            var e = await _repo.GetBySlugWithTreeAsync(slug, ct);
            return e is null ? null : e.ToResponse().ToOrganogramaResponse();
        }

        public async Task<List<OrganogramaResponse>> SearchAsync( string? ativo, DateTime? publicadoAte, CancellationToken ct = default)
        {
            var list = await _repo.SearchAsync(ativo, publicadoAte, ct);
            return list.Select(x => x.ToResponse()).ToOrganogramaResponse();
        }

        public async Task<long> CreateAsync(CreateOrganogramaRequest req, CancellationToken ct = default)
        {
            var instCreate = req.ToInstitucionalCreate();

            // slug único (gera do título se vazio)
        ...
        public async Task UpdateAsync(long id, UpdateOrganogramaRequest req, CancellationToken ct = default)
        {
